...
#define easy_hash_for_each_safe(i, node, next_node, table)                  \
    for(i=0; i<table->size; i++)                                       \
        for(node = table->buckets[i]; node && ({ next_node = node->next; 1; }); node = next_node)

#define easy_hash_entry(table, node)                                   \
...
extern easy_string_pair_t *easy_hash_string_next(easy_hash_string_t *table, easy_string_pair_t *t);
