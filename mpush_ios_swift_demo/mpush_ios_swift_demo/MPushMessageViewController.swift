...
class MPushMessageViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            // 数据库中删除该条记录
            let dao : PushMessageDAO = PushMessageDAO.init()
            dao.remove(model: pushMessage!.object(at: indexPath.row) as! RPushMessage)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let tablecell: RPushMessage = pushMessage!.object(at: indexPath.row) as! RPushMessage
        cell.textLabel!.text = tablecell.messageContent as String?
        if tablecell.isRead! {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let tappedItem :RPushMessage = pushMessage?.object(at: indexPath.row) as! RPushMessage
        if (tappedItem.isRead)! {
        } else {
            tappedItem.isRead = true;
            let dao : PushMessageDAO = PushMessageDAO.init()
            dao.update(model: tappedItem)
            refreshTable()
        }
    }
}
